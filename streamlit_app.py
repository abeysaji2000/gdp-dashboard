...
# -----------------------------------------------------------------------------
# Filters
age_range = st.sidebar.slider("Filter by Age Range", int(df["Age"].min()), int(df["Age"].max()), (20, 50))
selected_gender = st.sidebar.selectbox("Filter by Gender", df["Gender"].unique())
...
fig_heatmap = px.imshow(corr_matrix, color_continuous_scale="Viridis", title="Correlation Heatmap")
